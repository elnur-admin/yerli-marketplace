...
function baseHeaders(req, extra = {}) {
  ...
  return { 'X-Content-Type-Options': 'nosniff', 'Referrer-Policy': 'same-origin', 'Cache-Control': 'no-store', ...(origin && isAllowedOrigin(origin) ? { 'Access-Control-Allow-Origin': origin, Vary: 'Origin' } : {}), ...extra };
}
...
async function api(db, req, res, url) {
  ...
  if (sellerProduct && req.method === 'PATCH') { const user = await requireUser(db, req); const store = await getStore(db, user.id); if (!store) fail(404, 'Mağaza tapılmadı'); const body = await readBody(req); const product = await db.get(`${productSelect} WHERE p.id=? AND p.store_id=?`, [sellerProduct[1], store.id]); if (!product) fail(404, 'Məhsul tapılmadı'); const stock = body.stock === undefined ? product.stock : integer(body.stock, 'Stok', 0, 100000); await db.run('UPDATE products SET stock=?,updated_at=? WHERE id=?', [stock, now(), product.id]); return send(req, res, 200, { product: productView(await db.get(`${productSelect} WHERE p.id=?`, [product.id])) }); }
